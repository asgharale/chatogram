...
class BotHandlers:
    ...
    def dispatch(self, user, chat_id: int, text, contact, photo, cb_data, message_id=None,
        ...
        if cb_data:
            ...
            elif cb_data == "share_referral_link":
                self.handle_share_referral_link(user, chat_id)
    ...
    def handle_referral(self, user, chat_id: int):
        ...
        send_key_message_task.delay(
            ...
            reply_markup={
                "inline_keyboard": [
                    [{"text": "📤 ارسال لینک معرفی", "callback_data": "share_referral_link"}],
                ]
            },
            parse_mode="Markdown",
        )

    def handle_share_referral_link(self, user, chat_id: int):
        """
        Sends the referral link as a plain, standalone message — does NOT
        open/follow the link. Plain URLs auto-link and are long-press
        copyable in Bale, and a bare message is the easiest to forward
        to a friend or paste somewhere else.
        """
        from .tasks import send_message_task
        code = user.referral_code or "---"
        url  = f"https://ble.ir/alochatbot?start={code}"
